...
async def lifespan(app: FastAPI):
    # Startup: warm-load the primary model; fallback is loaded lazily on first use.
...
_EXPLICIT_TEXT_REGEX = re.compile(
    "|".join(EXPLICIT_TEXT_PATTERNS), re.IGNORECASE)
...
def _load_custom_toxic_model():
    ...
    if os.path.exists(MODEL_PATH):
        try:
            ...
            print(
                f"[main.py] Failed to load custom model ({exc}); will use fallback.")
...
def _load_detoxify_fallback():
    ...
    try:
        # Import lazily and dynamically so Detoxify remains an optional dependency.
        import importlib
...
def _load_nsfw_image_model():
    ...
    try:
        from nudenet import NudeClassifier # pyright: ignore[reportMissingImports]
...
def score_toxicity(text: str) -> float:
    ...
    if model is not None:
        try:
            ...
            print(
                f"[main.py] Primary model inference failed ({exc}); falling back.")
    ...
    crude_flags = ["idiot", "stupid", "hate you",
                   "kill yourself", "pathetic", "worthless"]
...
def is_image_nsfw(image_url: str, confidence_threshold: float = 0.60) -> bool:
    ...
    try:
        ...
        # Unique per-call filename (not a fixed shared name) so concurrent
        # requests — FastAPI runs these sync endpoints in a thread pool —
        # never read or delete each other's in-flight file.
...
def suggest_rephrase(text: str) -> str:
    ...
    replacements = [
        ...
        (r"\byou'?re\s+(an?\s+)?(moron|stupid|dumb)\b",
         "I think you're mistaken here"),
        (r"\byou\s+are\s+(an?\s+)?(moron|stupid|dumb)\b",
         "I think you're mistaken here"),
        ...
        (r"\byou\s+are\s+(pathetic|worthless|garbage|trash)\b",
         "I don't agree with this at all"),
        (r"\bshut\s*up\b", "please stop for a second"),
    ]
    rephrased = text
    for pattern, replacement in replacements:
        rephrased = re.sub(pattern, replacement,
    ...
    if rephrased == text:
        ...
        rephrased = re.sub(
            r"\b[A-Z]{4,}\b", lambda m: m.group(0).title(), rephrased)
        if rephrased == text:
            rephrased = (
...
def train_custom_classifier():
    ...
    return {
        ...
        "log": result.stdout.strip().splitlines()[-6:],
    }
